...
const QuestionOwnerPanel: React.FC<Props> = ({
  questionId,
  reviewStatus,
  reviewMessage,
  reviewTime,
  ...
  const statusSummary = useMemo(() => {
    if (status === QUESTION_REVIEW_STATUS_ENUM.PRIVATE) {
      return {
        title: "当前题目仍是私有草稿",
        description: "这道题现在只有你和管理员能看到。准备公开时，再主动提交审核会更稳。",
    ...
    if (status === QUESTION_REVIEW_STATUS_ENUM.PENDING) {
      return {
        title: "题目已提交审核",
        description: "审核通过前，这道题还不会出现在公开题目列表、推荐位和他人可见页面里。",
    ...
    return {
      title: "题目已经公开",
      description: "现在其他用户可以在公开题目列表、推荐链路和相关题库里看到它。",
  ...
  const handleSubmitReview = async () => {
    ...
    setSubmitting(true);
    const hide = message.loading("正在提交题目审核");
    try {
    ...
    <section className="rounded-[2.5rem] border border-slate-100 bg-white p-8 shadow-2xl shadow-slate-200/30">
      <div className="flex flex-col gap-6 lg:flex-row lg:items-start lg:justify-between">
        <div className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            <Paragraph className="!mb-0 max-w-3xl text-sm leading-7 text-slate-500">
              {isOwner ? statusSummary.description : "你当前正在以管理员身份查看这道题目。"}
            </Paragraph>
          </div>
        </div>
        ...
        <Space wrap size="middle">
          ...
          {isOwner ? (
            <Link href="/user/center?tab=submission">
