...
export default function MyFavourPostList() {
  ...
  const fetchPostList = useCallback(async (page = 1, nextKeyword = "", nextSortKey: SortKey = "latest_update") => {
    ...
    try {
      ...
      message.error(error?.message || "获取收藏帖子失败");
    } finally {
      setLoading(false);
    }
  }, [pageSize]);
        ...
        <div className="space-y-5">
          ...
          <div className="flex justify-end pt-2">
            <Pagination
              ...
              onChange={(page) => {
                void fetchPostList(page, keyword, sortKey);
              }}
            />
          </div>
        </div>
          ...
          <Empty
            ...
            description={
              <div className="space-y-2">
                <div className="text-base font-bold text-slate-700">还没有收藏过帖子</div>
                <div className="text-sm text-slate-400">遇到优质经验帖时点一下收藏，后面回顾会方便很多。</div>
              </div>
            }
          />
          <Link
